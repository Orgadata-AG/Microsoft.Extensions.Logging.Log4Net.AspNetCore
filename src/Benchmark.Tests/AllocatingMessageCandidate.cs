...
namespace Microsoft.Extensions.Logging.Log4Net.AspNetCore.Entities
{
    ...
    public class AllocatingMessageCandidate<TState>
    {
        public AllocatingMessageCandidate(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            State = state;
            LogLevel = logLevel;
            EventId = eventId;
